# RFC 68: responsive images support in Wagtail core
...
> 🚧 A prototype of this proposal is available for review and testing: [wagtail_picture_proposal](https://github.com/torchbox/wagtail_picture_proposal).
...
See [refactoring of the bakerydemo homepage](https://github.com/thibaudcolas/bakerydemo/pull/3).
...
```html
<img
  {% if srcset|length > 1 %}srcset="{% for source in srcset %}
      {{ source.url }} {{ source.width }}w{% if not forloop.last %},{% endif %}{% endfor %}"{% endif %}
  src="{{ srcset.0.url }}"
  width="{{ srcset.0.width }}"
  height="{{ srcset.0.height }}"{% for name, value in attributes.items %}
  {{ name }}="{{ value }}"{% endfor %}
>
```
...
Here are reports of applying responsive images and WebP `picture` tags to existing sites, to compare the size of image payloads before/after. For the performance of the page’s generation, we expect any change to be minimal since:
